...
const Dashboard = () => {
    ...
    const fetchMyItems = async () => {
        try {
            ...
            // Try to use the specific getMyItems endpoint first
            try {
                const response = await itemsAPI.getMyItems();
                ...
                // Fallback: fetch all items with pagination and filter client-side
                // Note: This is not optimal but works if getMyItems endpoint doesn't exist
                console.warn("getMyItems failed, fetching all items:", err);
                let allItems = [];
                let currentPage = 1;
            ...
            try {
                ...
                if (error.response) {
                    ...
                    if (error.response.status === 403) {
                        setError(
                            "🚫 Access Denied: You can only delete items that you created."
                        );
                    } else if (error.response.status === 404) {
                        setError(
                            "🔍 Item Not Found: The item may have been already deleted."
                        );
